...
void QGraphicsItemPrivate::updateSceneTransformFromParent()
{
    if (parent) {
        ...
        sceneTransformTranslateOnly = true;
...
void QGraphicsItem::setFlags(GraphicsItemFlags flags)
{
    ...
    if ((flags & ItemClipsChildrenToShape) != (oldFlags & ItemClipsChildrenToShape)) {
        ...
        d_ptr->dirtyChildrenBoundingRect = true;
    ...
    if ((flags & ItemStacksBehindParent) != (oldFlags & ItemStacksBehindParent)) {
        ...
        if (d_ptr->parent)
            d_ptr->parent->d_ptr->needSortChildren = true;
        else if (d_ptr->scene)
...
{
    CacheMode lastMode = d_ptr->cacheMode;
...
void QGraphicsItemPrivate::setVisibleHelper(bool newVisible, bool explicitly, bool update)
{
    ...
    if (!newVisible) {
        ...
        paintedViewBoundingRectsNeedRepaint = true;
        if (scene) {
            if (isWidget) {
...
{
    return d_ptr->acceptedMouseButtons;
}
...
{
    if (d_ptr->acceptsHover == enabled)
        return;
    d_ptr->acceptsHover = enabled;
...
QRectF QGraphicsItem::childrenBoundingRect() const
{
    ...
    d_ptr->dirtyChildrenBoundingRect = false;
    return d_ptr->childrenBoundingRect;
}
...
void QGraphicsItem::setBoundingRegionGranularity(qreal granularity)
{
    ...
    if (granularity == 0.0) {
        ...
        d_ptr->hasBoundingRegionGranularity = false;
        return;
    }
    d_ptr->hasBoundingRegionGranularity = true;
...
void QGraphicsItemPrivate::addChild(QGraphicsItem *child)
{
    ...
    needSortChildren = true; // ### maybe false
