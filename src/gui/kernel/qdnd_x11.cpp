...
static const unsigned char default_pm_kat[] = {
  0x4b, 0x41, 0x54, 0x49, 0x45, 0x04, 0x00, 0x0d, 0x00, 0x09, 0x00, 0x00,
  0x01, 0xd4, 0x00, 0x00, 0x00, 0x34, 0x78, 0x01, 0xbd, 0x8a, 0xc1, 0x09,
  0x00, 0x40, 0x0c, 0xc2, 0xdc, 0x7f, 0x69, 0xaf, 0x15, 0xf1, 0xeb, 0xbd,
  0x1a, 0x68, 0x03, 0x12, 0x92, 0x04, 0x30, 0xff, 0xc6, 0x73, 0x3f, 0xa4,
  0xb7, 0x2b, 0xee, 0x56, 0xb2, 0xc6, 0x4e, 0x7a, 0xbb, 0xe2, 0x6e, 0x25,
  0x6b, 0xec, 0xa4, 0xbf, 0xf0, 0x03, 0x12, 0xc4, 0xb3, 0x4d
};
static const unsigned int default_pm_kat_len = 70;
...
void QX11Data::xdndHandleDrop(const XEvent * xe, bool passive)
{
    ...
    if (!passive) {
        ...
        if (at != -1) {
            ...
            // Can't use the source QMimeData if we need the image conversion code from retrieveData_sys
...
QVariant QDropData::retrieveData_sys(const QString &mimetype, QVariant::Type requestedType) const
{
    ...
    QByteArray encoding;
    Atom a = qt_x11Data->xdndMimeAtomForFormat(mimetype, requestedType, atoms, &encoding);
    if (!a)
        return result;
