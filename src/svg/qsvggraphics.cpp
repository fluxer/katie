      ...
      m_bounds(bounds)
{
    if (m_bounds.width() == 0.0)
        m_bounds.setWidth(qreal(m_image.width()));
    if (m_bounds.height() == 0.0)
        m_bounds.setHeight(qreal(m_image.height()));
}
...
void QSvgText::draw(QPainter *p, QSvgExtraStates &states)
{
    ...
    if (m_type == TEXTAREA) {
        if (states.textAnchor == Qt::AlignHCenter)
            px += scaledSize.width() / 2;
        else if (states.textAnchor == Qt::AlignRight)
            px += scaledSize.width();
    }
