...
void tst_QComboBox::textpixmapdata()
{
    ...
    for (int i = 0; i<text.count(); ++i) {
        ...
        QVERIFY(icon.cacheKey() == icons.at(i).cacheKey());
...
void tst_QComboBox::itemListPosition()
{
    ...
    const QRect screen = QApplication::desktop()->availableGeometry(scrNumber);
...
#else
    QSKIP("Katie compiled without cleanlooks style (QT_NO_STYLE_CLEANLOOKS)", SkipAll);
#endif
}
...
void tst_QComboBox::task260974_menuItemRectangleForComboBoxPopup()
{
    ...
    QSKIP("Katie compiled without windows style (QT_NO_STYLE_WINDOWS)", SkipAll);
#else
