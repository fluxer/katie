...
void tst_QRegExp::exactMatch()
{
    ...
    for ( int i = 0; i < 65536; i++ ) {
        ...
        bool is_w = ( ch.isLetterOrNumber() || ch.isMark() || ch == '_' );

        QCOMPARE( rx_d.exactMatch(QString(ch)), is_d );
        QCOMPARE( rx_s.exactMatch(QString(ch)), is_s );
        QCOMPARE( rx_w.exactMatch(QString(ch)), is_w );
